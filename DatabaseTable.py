...
class InfoPool(object):
    def __init__(self,table_name):
        self.__table_name = table_name
        self.__database_name = "InfoPool.db"   ##数据库名称为InfoPool.db
    ##初始化类，传入参数table_name
    ...
    def create(self):
        ...
        conn.execute(
            ...
            % self.__table_name)
        print("InfoPool.db数据库下%s表建表成功" % self.__table_name)
    ##建表语句    
    ...
    def delete(self, TIME = '1', delete_all=False):
        conn = sqlite3.connect(self.__database_name,isolation_level = None)
        if not delete_all:
            n = conn.execute("delete from %s where TIME=?" % self.__table_name,
                        (TIME,))
            #逗号不能省，元组元素只有一个的时候一定要加
            print("删除了",n.rowcount,"行记录")
        else:
